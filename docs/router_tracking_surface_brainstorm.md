...
>
> **Live-wiring note (verified):** `route()` IS wired live at `scanners/intraday_scanner.py:211`, inside a `config.INTRADAY_PAPER_BROKER_ENABLED` block (currently **`True`** — router live during market hours) (broker constructed at line 210, call exception-isolated at line 218). `paper_broker.py` does not call the router. So the spec is two-phase: **Phase 1** = offline `route_explain()` + backfill + rollup (unblocks calibration, no live change); **Phase 2 (DEFERRED)** = swap `_route_entry`→`route_explain` at that existing seam + live trace write — purely additive (no broker injection, no behavior change; same accepted set executed), emitting rows only while the flag is on. **User direction 2026-05-30: Offline-only v1 — build Phase 1, defer Phase 2.**
